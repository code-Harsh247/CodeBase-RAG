...
| Free-tier LLM quota blocks development or corrupts the eval run | Gemini's free tier (20 requests/day) was ruled out before any agent code was written. Groq's free tier (1,000/day, an undocumented 200k tokens/day cap found in Phase 3) covered development through Phase 3 but ran out mid-session in the web UI, so it was dropped; OpenRouter (`qwen/qwen3-coder`, ~$0.005/question, under $0.25 for the full eval sweep) is now the sole provider, and no result is silently degraded by a quota. See ARCHITECTURE.md §2.4a. |
| A cheap or free model quietly weakens graph-hybrid specifically | The naive-RAG baseline never has to generate Cypher, so a model that struggles at structured generation handicaps only the system this project argues for, not the comparison as a whole. Measured directly: a candidate free local model (Ollama, `llama3.1:8b`) wrote invalid, unscoped Cypher on a real eval-style question. `qwen/qwen3-coder` was chosen after it produced correct, schema-aware Cypher on the same question. |
