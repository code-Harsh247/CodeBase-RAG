...
| Free-tier LLM quota blocks development or corrupts the eval run | Gemini's free tier (20 requests/day) was ruled out before any agent code was written. Groq's free tier (1,000/day, an undocumented 200k tokens/day cap found in Phase 3) covers development; the Phase 4 scored run uses a pinned model on OpenRouter (`qwen/qwen3-coder`, under $0.25 for the full sweep) instead, so the headline result is never silently degraded by a quota. See ARCHITECTURE.md §2.4a. |
